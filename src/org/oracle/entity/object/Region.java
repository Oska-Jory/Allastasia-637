...
public class Region {
	...
	public static List<User> getLocalPlayers(Location tile) {
		return getLocalPlayers(tile, 16);
	}

	public static List<User> getLocalPlayers(Location tile, int depth) {
		int baseX = tile.getX();
		int baseY = tile.getY();
		int z = tile.getZ();
		int lastRegionId = hash(baseX, baseY);
...
/*	public static List getRegionTiles(Location tile, int depth, int type) {
	...
	private Set<User>[] players = new HashSet[4];
	@SuppressWarnings("unchecked")
